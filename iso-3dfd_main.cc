...
typedef struct\{\
        ...
        int num_threads;\
        int nreps;      // number of time-steps, over which performance is aver\\\
aged                                                                            \
...
void initialize(float* ptr_prev, float* ptr_next, float* ptr_vel, Parameters* p\\\
, size_t nbytes)\{\
...
\
\
Void initializeFT(float* ptr_prev, float* ptr_next, float* ptr_vel, Parameters*\\\
 p, size_t nbytes, int n1_Tblock, int n2_Tblock, int n3_Tblock, int nThreads)\{\
\
...
\{\
                                                       ptr_line_next = &ptr_nex\\\
t[iz*dimn1n2 + iy*n1 + bx];\
                                                        ptr_line_prev = &ptr_pr\\\
ev[iz*dimn1n2 + iy*n1 + bx];\
                                                        ptr_line_vel = &ptr_vel\\\
[iz*dimn1n2 + iy*n1 + bx];\
\
                                                        #pragma ivdep\
                                                        for(int ix=0; ix<ixEnd;\\\
 ix++) \{\
\
                                                                ptr_line_prev[i\\\
x] = 0.0f;\
...
x] = 0.0f;\
                                                                ptr_line_vel[ix\\\
] = 2250000.0f*DT*DT;//Integration of the v\'b2 and dt\'b2 here                       \
                                                        \}\
                                                \}\
                                        \}\
                                \}\
...
\{\
        ...
        p.num_threads = 24;\
        p.nreps = 100;     // number of time-steps, over which performance is a\\\
veraged                                                                         \
...
\
        if( (argc > 1) && (argc < 4) ) \{\
                printf(" usage: [n1 n2 n3] [# threads] [# iterations] [thread b\\\
lock n1] [thread block n2] [thread block n3]\\n");\
                exit(1);\
        \}\
        // [n1 n2 n3]                                                           \
        if( argc >= 4 ) \{\
	...
	\}\
\
	// Make sure n1 and n1_Tblock are multiple of 16 (to support 64B alignm\\\
ent)                                             \
        if ((p.n1%16)!=0) \{\
...
1,p.n2,p.n3,p.nreps,p.num_threads,HALF_LENGTH);\
	printf("n1_thrd_block=%d n2_thrd_block=%d n3_thrd_block=%d\\n", p.n1_Tbl\\\
ock, p.n2_Tblock, p.n3_Tblock);\
\
...
16))/(1024*1024));fflush(NULL);\
\
\
#ifndef USE_HBM\
        float *prev_base = (float*)_mm_malloc( (nsize+16+MASK_ALLOC_OFFSET(0 ))\\\
*sizeof(float), CACHELINE_BYTES);\
        float *next_base = (float*)_mm_malloc( (nsize+16+MASK_ALLOC_OFFSET(16))\\\
*sizeof(float), CACHELINE_BYTES);\
        float *vel_base  = (float*)_mm_malloc( (nsize+16+MASK_ALLOC_OFFSET(32))\\\
*sizeof(float), CACHELINE_BYTES);\
#else\
        float *prev_base, *next_base, *vel_base;\
        hbw_posix_memalign((void**)(&(prev_base)), CACHELINE_BYTES,  (nsize+16+\\\
MASK_ALLOC_OFFSET(0))*sizeof(float));\
        hbw_posix_memalign((void**)(&(next_base)), CACHELINE_BYTES,  (nsize+16+\\\
MASK_ALLOC_OFFSET(16))*sizeof(float));\
        vel_base  = (float*)_mm_malloc( (nsize+16+MASK_ALLOC_OFFSET(32))*sizeof\\\
(float), CACHELINE_BYTES);\
#endif\
\
\
        if( prev_base==NULL || next_base==NULL || vel_base==NULL )\{\
                printf("couldn't allocate CPU memory prev_base=%p next=_base%p \\\
...
#ifdef USE_FT\
\
       initializeFT(p.prev, p.next, p.vel, &p, nbytes, p.n1_Tblock, p.n2_Tblock\\\
, p.n3_Tblock, p.num_threads);\
\
#else\
...
\
 ...
 tmp_nreps, p.n1_Tblock, p.n2_Tblock, p.n3_Tblock);\
\
\
\
// real measurements start here                                                 \
\
        wstart = walltime();\
	iso_3dfd(p.next, p.prev, p.vel, coeff, p.n1, p.n2, p.n3, p.num_threads,\\\
...
\
	throughput_mpoints = ((p.n1-2*HALF_LENGTH)*(p.n2-2*HALF_LENGTH)*(p.n3-2\\\
*HALF_LENGTH))/(normalized_time*1e6f);\
\
...
\
        reference_implementation( p_ref, p.prev, coeff, p.vel, p.n1, p.n2, p.n3\\\
, HALF_LENGTH );\
        if( within_epsilon( p.next, p_ref, p.n1, p.n2, p.n3, HALF_LENGTH, 0, 0.\\\
0001f ) ) \{\
